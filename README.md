...
当前推荐主线是 **五指抓取到少指抓取的责任缺口补偿**：

1. 用五指 seed 计算每根手指对物体 patch 的责任；
2. 禁用部分手指后得到 disabled responsibility gap；
3. 对每根 active finger 顺序选择 residual targets；
4. 用 distal/middle/proximal 多假设 QP 让当前手指补偿缺口，同时控制穿深、自碰撞和关节变化；
5. thumb 和 palm/wrist 作为单独阶段做支撑与对齐。

也就是说，主方法不是 contact-free，也不是让所有 residual patch 同时硬约束接触；它是一个“责任缺口 -> 可执行 target -> 单指局部 QP -> acceptance guard”的逐步优化方法。
...
2. `docs/ZH_OPTIMIZATION_STEP_DETAILS_2026_07_01.md`
   - 当前推荐主方法的逐步优化明细；
   - 每一步输入、变量、目标项、约束、接受条件、输出；
   - 对应到 F2M 当前模块化代码。

3. `main.py`
   - 命令行入口，理解 `responsibility` 和 `sequential_qp` 两种运行模式。

4. `method/contact_targets.py`、`method/sequential_qp.py`、`method/qp_terms.py`
   - 当前方法最核心的三个文件。

5. `configs/cfet_default.json`
   - 当前推荐的稳定配置；
   - active finger 阶段使用 residual target selection + point-tracking QP；
   - `configs/cfet_gap_v2.json` 和 `configs/cfet_candidate_pool_v5.json` 保留为实验分支，不作为当前主结果。
...
- `visualizer/`：HTML/PLY/GLB 可视化；当前交互 viewer 只推荐看 GLB。
- `results/`：所有输出结果。
...
- `preview.html`：优化前后 HTML 对比页。
- `interactive_viewer.html`：BODex 风格的交互式 viewer，使用 `model-viewer` 加载 GLB。
- `<mode>/<mode>.glb`：每个 mode 的 3D 资产；PNG/GIF 不作为后续推荐输出。
